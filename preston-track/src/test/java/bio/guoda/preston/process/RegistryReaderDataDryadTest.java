...
public class RegistryReaderDataDryadTest {
    ...
    public void fromVersionsToFiles() {
        ...
        assertThat(statements.get(1).getSubject().ntriplesString(), is("<foo:bar>"));
    ...
    public void fileToEndpoints() {
        ...
        assertThat(statements.get(2).getSubject().ntriplesString(), is("<https://datadryad.org/api/v2/files/3985003/download>"));
        assertThat(statements.get(2).getPredicate().ntriplesString(), is("<http://purl.org/dc/elements/1.1/format>"));
        assertThat(statements.get(2).getObject().ntriplesString(), is("\"application/vnd.ms-excel\""));
        assertThat(statements.get(3).getSubject().ntriplesString(), is("<https://datadryad.org/api/v2/files/3985003/download>"));
        assertThat(statements.get(3).getPredicate().ntriplesString(), is("<http://purl.org/pav/hasVersion>"));
        assertThat(statements.get(3).getObject().ntriplesString(), is("<hash://sha256/dff4a33ec5fe8ade65c2d157048a9a99b537c316dea6329eb4f23775d2e8f79e>"));
        assertThat(statements.get(4).getSubject().ntriplesString(), is("<https://datadryad.org/api/v2/files/3985003/download>"));
        assertThat(statements.get(4).getPredicate().ntriplesString(), is("<http://purl.org/pav/hasVersion>"));
        assertTrue(RefNodeFactory.isBlankOrSkolemizedBlank(statements.get(4).getObject()));
        Quad lastStatement = statements.get(statements.size() - 1);
        assertThat(lastStatement.getSubject().ntriplesString(), is("<https://datadryad.org/api/v2/files/3985010/download>"));
        assertThat(lastStatement.getPredicate().ntriplesString(), is("<http://purl.org/pav/hasVersion>"));
        assertThat(lastStatement.getObject().ntriplesString(), startsWith("_:"));
    }
