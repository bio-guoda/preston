...
public class HashGeneratorImplTest {
    ...
    public void hashTextFromResource() throws IOException {
        ...
        IRI hash = new HashGeneratorImpl(HashType.sha256).hash(is);
    ...
    public void hashTextFromString() throws IOException {
        IRI hash = new HashGeneratorImpl(HashType.sha256).hash(IOUtils.toInputStream("hello", StandardCharsets.UTF_8));
    ...
    public void hashBinary() throws IOException {
        IRI hash = new HashGeneratorImpl(HashType.sha256).hash(getClass().getResourceAsStream("/bio/guoda/preston/dwca-20180905.zip"));
