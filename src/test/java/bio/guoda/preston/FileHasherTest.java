...
public class FileHasherTest {
    ...
    public void hashOfOrderedHashes() throws IOException, URISyntaxException {
        ...
        Map<Long, File> hashFiles = new TreeMap<>();
        ...
        while ((entry = is.getNextEntry()) != null) {
            ...
            hashFiles.put(entry.getCrc(), tmpFile);
            String iriString = Hasher.calcSHA256(is, IOUtils.buffer(os)
                    , false).getIRIString();
            hashList.add(iriString);
            FileUtils.moveFile(tmpFile, FilePersistence.getDataFile(dataDir, new KeyTo5LevelPath().toPath(iriString)));
            is.closeEntry();
        }
        is.close();
        ...
        String joined = sortedHashes.collect(Collectors.joining());
        IRI multiHash = Hasher.calcSHA256(joined);
        ...
        assertThat(Arrays.asList(hashList.toArray()), is(expectedHashes));
        assertThat(joined, startsWith("hash://sha256/066e685fa9b5ef8a01d7d69f2f0e7f4074a646e5ae437f741a4629d881ade9eb"));

        String hashOfSortedHash = "hash://sha256/057391bd7ebe300dca56b8c2f44ec5526b8986bf832be552f285ad7d700aa1e6";
        assertThat(multiHash.getIRIString(), is(hashOfSortedHash));

        String joinedUnsorted = hashList.stream().collect(Collectors.joining());

        IRI multihashUnsorted = Hasher.calcSHA256(joinedUnsorted);
        assertThat(multihashUnsorted.getIRIString(), is(not(hashOfSortedHash)));
